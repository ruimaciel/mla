...
BOOST_AUTO_TEST_CASE_TEMPLATE( MatrixCursor_check_end_of_column, Scalar, scalar_list )
{
	using namespace mla::matrix;
	...
	cursor.increment_column();
	cursor.increment_column();
	BOOST_CHECK( !cursor.at_end_of_current_column() );
	cursor.increment_column();
	BOOST_CHECK( cursor.at_end_of_current_column() );
}


BOOST_AUTO_TEST_CASE_TEMPLATE( MatrixCursor_check_end_of_row, Scalar, scalar_list )
{
	using namespace mla::matrix;
	...
	cursor.increment_row();
	cursor.increment_row();
	BOOST_CHECK( !cursor.at_end_of_current_row() );
	cursor.increment_row();
	BOOST_CHECK( cursor.at_end_of_current_row() );
}
